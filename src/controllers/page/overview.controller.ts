...
async function getAnimeData(mal_id: number) {
	return mediaLocks(mal_id, async () => {
		...
		try {
			...
			const relAdaptation = animeRelation.relation_data.find(
				(rel) => rel.relation === 'Adaptation'
			);
...
async function getMangaData(mal_id: number) {
	return mediaLocks(mal_id, async () => {
		...
		try {
			...
			const relAdaptation = mangaRelation.relation_data.find(
				(rel) => rel.relation === 'Adaptation'
			);
