...
export async function seedTableAnimeTrending(trendingAnimeList: Anime[]): Promise<void> {
	...
	try {
      ...
      `,
		...
		const checkDatabase = await client.query(`SELECT id FROM anime_trending`);

		console.log(
			`Successfully inserting trending anime data list (${checkDatabase.rowCount} / ${trendingAnimeList.length})`
		);
	} catch (err) {
