...
export async function createTableTrendingAnime() {
  ...
  CREATE TABLE anime_trending (
    ...
    CREATE INDEX idx_anime_mal_id ON anime_trending(mal_id);
    CREATE INDEX idx_anime_score ON anime_trending(score);
    CREATE INDEX idx_anime_type ON anime_trending(type);
    CREATE INDEX idx_anime_data ON anime_trending USING GIN (data);
  `
	);
