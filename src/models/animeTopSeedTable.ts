...
export async function seedTableAnimeTop(topAnimeList: Anime[]): Promise<void> {
	...
	try {
      ...
      `,
		...
		const checkDatabase: QueryResult<Anime[]> = await client.query(`SELECT * FROM anime_top`);

		console.log(`Successfully inserting top anime data list (${checkDatabase.rowCount} / ${topAnimeList.length})`);
	} catch (err) {
