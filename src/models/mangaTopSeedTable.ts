...
export async function seedTableMangaTop(topMangaList: Manga[]): Promise<void> {
	...
	try {
		...
		const checkDatabase: QueryResult<Manga[]> = await client.query(`SELECT * FROM manga_top`);

		console.log(`Successfully inserting top anime data list (${checkDatabase.rowCount} / ${topMangaList.length})`);
	} catch (err) {
