...
async function fetchTitle(page, limit) {
	try {
		...
		const res = await fetch(`/api/trending/anime?page=${page}&limit=${limit}`);
...
function renderTitle(list) {
	if (!list || list.length === 0) {
		listContainer.innerHTML = `
            <div class="col-span-full text-center py-8">
                <p class="text-lg opacity-70">No anime found</p>
            </div>
        `;
		return;
	}
	...
	const cards = list
		.map((anime) => {
			return `
                <a href="/overview/anime/${anime.mal_id}" class="group">
...
function updatePageInfo(currentPage, totalPages, totalRecords) {
	...
	pageInfo.innerHTML = `
       	<p class="text-base">
						Page ${currentPage} of ${totalPages}
						| Showing ${startItem}-${endItem} of ${totalRecords} anime
					</p>
    `;
}
